        ...
        public static Task<MergeResult> MergeAsync(string targetPath)
        {
            return Task.Run(() =>
            {
                ...
                if (File.Exists(targetPath))
                {
                    ...
                    if (ext != ".xls" && ext != ".xlsx")
                        throw new InvalidOperationException("Chỉ hỗ trợ .xls / .xlsx");
                    ...
                    var rep = NpoiMerger.MergeFast_ToOneXlsx(
                        new[] { targetPath },
                        output,
                        trySkipHeaderIfMatchesBase: true,
                        AppLog.Write);
                ...
                if (Directory.Exists(targetPath))
                {
                    ...
                    if (files.Length == 0)
                        throw new InvalidOperationException("Folder không có file .xls/.xlsx hợp lệ!");

                    var output = MakeUnique(Path.Combine(targetPath, "merged_all.xlsx"));

                    // tránh merge nhầm output cũ
                    files = files.Where(f => !string.Equals(Path.GetFullPath(f), Path.GetFullPath(output), StringComparison.OrdinalIgnoreCase))
                                 .ToArray();
                ...
                throw new FileNotFoundException("Không tìm thấy file hoặc folder: " + targetPath);
            });
        }
